...
class DependencyInstaller:
    ...
    def install_tesseract(self) -> bool:
        ...
        if self.platform == "darwin":  # macOS
            ...
            # Install tesseract
            code, stdout, stderr = self.run_command(["brew", "install", "tesseract"])
            if code != 0:
            ...
            for lang in ["eng", "fra", "deu", "spa", "chi_sim", "jpn", "kor"]:
                self.run_command(["brew", "install", f"tesseract-lang-{lang}"], check=False)
        ...
        # Verify installation
        code, stdout, _ = self.run_command(["tesseract", "--version"])
        if code == 0:
            self.log(f"Tesseract installed successfully", "SUCCESS")
            if self.verbose:
                print(stdout.split('\n')[0])
            return True
    ...
    def check_cuda(self) -> bool:
        ...
        code, stdout, _ = self.run_command(["nvidia-smi"], check=False)
        if code == 0:
            if self.verbose:
    ...
    def install_all(self, skip_tesseract: bool = False, skip_clip: bool = False,
                   skip_face: bool = False) -> bool:
...
def main():
    ...
    parser.add_argument(
        "--skip-tesseract",
        action="store_true",
        help="Skip Tesseract OCR installation"
    )
    parser.add_argument(
        "--skip-clip",
        action="store_true",
        help="Skip CLIP installation"
    )
    parser.add_argument(
        "--skip-face",
        action="store_true",
        help="Skip InsightFace installation"
    )
    parser.add_argument(
        "--verify-only",
        action="store_true",
    ...
    print("=" * 60)
    print("Ideal Goggles ML Dependencies Installer")
    print("=" * 60)
    print()
    ...
    if args.verify_only:
        ...
        if all_installed:
            print("\n✅ All ML dependencies are installed!")
        else:
            print("\n⚠️  Some dependencies are missing. Run without --verify-only to install.")

        return 0 if all_installed else 1

    # Perform installation
    print(f"Platform: {installer.platform} ({installer.arch})")
    ...
    if installer.install_all(
        ...
        print("-" * 30)
        for component, installed in results.items():
            status = "✅ Installed" if installed else "⚠️  Not verified"
            print(f"  {component:15} {status}")

        print("\n✅ ML dependencies are ready to use!")
        print("\nYou can now:")
        print("  - Use OCR for text extraction from images")
        print("  - Perform semantic search with natural language")
        print("  - Search by faces and manage people")
