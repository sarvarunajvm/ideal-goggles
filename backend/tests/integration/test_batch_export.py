...
class TestBatchExport:
    ...
    @pytest.mark.asyncio
    async def test_export_1k_photos_performance(self, batch_worker, mock_photos, temp_export_dir):
        ...
        with patch('src.workers.batch_worker.get_session') as mock_session:
            ...
            with patch('aiofiles.open', new_callable=AsyncMock) as mock_aiofiles:
                ...
                assert photos_per_minute >= 100, f"Export too slow: {photos_per_minute:.1f} photos/min"
    ...
    @pytest.mark.asyncio
    async def test_export_with_metadata(self, batch_worker, mock_photos, temp_export_dir):
