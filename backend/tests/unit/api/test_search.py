...
class TestSearchCoverage(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patches = [
            patch(
                "src.api.search.get_database_manager", return_value=self.mock_db_manager
            ),
...
class TestTextSearch(TestSearchCoverage):
    def test_search_photos_all_filters(self):
        ...
        result = asyncio.run(
            self.search_module.search_photos(
                q="photo",
    ...
    def test_search_photos_no_query(self):
        ...
        asyncio.run(
            self.search_module.search_photos(
                q=None, from_date=None, to_date=None, folder=None, limit=10, offset=0
            )
        )
...
class TestSemanticSearch(TestSearchCoverage):
    def setUp(self):
        ...
        self.mock_embedding_worker.generate_text_embedding = AsyncMock(
        ...
        self.worker_patch = patch(
            ...
            self.mock_embedding_worker_cls,
        )
    ...
    def test_semantic_search_success(self, mock_execute):
        ...
        self.mock_embedding_worker.generate_text_embedding.assert_called_with(
            "test query"
        )
...
class TestExecutionSemanticSearch(TestSearchCoverage):
    def test_faiss_search_success(self):
        ...
        mock_service.index.search.return_value = (
            np.array([[0.1, 0.2]]),
            np.array([[0, 1]]),
        )
        ...
        results = asyncio.run(
            self.search_module._execute_semantic_search(
...
class TestImageSearch(TestSearchCoverage):
    ...
    def test_image_search_success(self):
        ...
        self.mock_embedding_worker.generate_embedding = AsyncMock(
            return_value=mock_emb_obj
        )

        with patch(
            "src.api.search._execute_image_search", return_value=[]
        ) as mock_exec:
            asyncio.run(self.search_module.image_search(file, top_k=10))
...
class TestFaceSearch(TestSearchCoverage):
    def test_face_search_disabled(self):
        ...
        with patch(
            "src.api.config._get_config_from_db",
            return_value={"face_search_enabled": False},
        ):
    ...
    def test_face_search_person_not_found(self):
        ...
        with patch(
            "src.api.config._get_config_from_db",
            return_value={"face_search_enabled": True},
        ):
            self.mock_db_manager.execute_query.return_value = []
    ...
    def test_success(self, _):
        ...
        with patch("src.api.search.FileResponse", MagicMock()) as mock_response:
            ...
            mock_response.assert_called_with(
                path="/real/path.jpg", media_type="image/jpeg", filename="path.jpg"
            )
